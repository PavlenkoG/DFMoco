...
/*	else if (!(strncmp((char *)(RxBuffer),"sq",2))){//send queue command
	...
	RxBuffer = RxBuffer + 2;
	if (data_present) {
		if (len < 3) userCmd.command = CMD_NONE;
		len = len - 3;
		...
		while (len) {
			if (((data >= '0' && data <= '9') || data == '-') && lastUserData == ' ')
			{
				...
				{
					HAL_UART_Transmit(&huart2, (uint8_t*)(err_comand), sizeof(err_comand)-1,100);
...
void processMessage (uint8_t MSG) {
	...
	switch (MSG)
  {
		...
		case MSG_GETSTAT:
			...
			if (param_16t == INACTIVE) param_16t_c_len = sprintf(parameter_c, "ms 0000\r\n");
			else param_16t_c_len = sprintf(parameter_c, "ms 1000\r\n");
		...
		case MSG_GET_POSITION:
			...
			if (userCmd.args[0]==0)userCmd.args[0]=1;
...
/*		case MSG_SETACCEL	:
	...
	if (int32_active)	HAL_UART_Transmit(&huart2, (uint8_t*)(parameter_c), param_32t_c_len,100);
	else HAL_UART_Transmit(&huart2, (uint8_t*)(parameter_c), param_16t_c_len,100);
	userCmd.args[0] = 0;
	userCmd.args[1] = 0;
